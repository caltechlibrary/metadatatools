(() => {
    ...
    l = class {
        ...
        this.inputEl.addEventListener("input", async (t) => {
          ...
          this.inputEl.addEventListener("keydown", (t) => {
            ...
            (++this.searchID,
              ...
              this.updateState("")), t.key === "Enter" && t.preventDefault();
          }),
      ...
      initContainer(e) {
        ...
        if (t.tagName === "INPUT") {
          ...
          }).text("Search this site").addTo(n),
            this.inputEl = new r("input").id(`pfmod-input-${s}`).class(
              "pagefind-modular-input",
            ).attrs({ autocapitalize: "none", enterkeyhint: "search" }).addTo(
              n,
            ),
            this.clearEl = new r("button").class("pagefind-modular-input-clear")
              .attrs({ "data-pfmod-suppressed": "true" }).text("Clear").handle(
                "click",
                () => {
                  this.inputEl.value = "",
                    this.instance.triggerSearch(""),
      ...
      register(e) {
        this.instance = e,
          this.instance.on("search", (t, s) => {
            this.inputEl && document.activeElement !== this.inputEl &&
  ...
  var g = (i) => {
    ...
    y = (i) => {
      ...
      i?.meta?.image &&
        new r("img").class("pagefind-modular-list-image").attrs({
          src: i.meta.image,
          alt: i.meta.image_alt || i.meta.title,
        }).addTo(t);
      let s = new r("div").class("pagefind-modular-list-inner").addTo(e),
        n = new r("p").class("pagefind-modular-list-title").addTo(s);
      return new r("a").class("pagefind-modular-list-link").text(i.meta?.title)
        .attrs({ href: i.meta?.url || i.url }).addTo(n),
        new r("p").class("pagefind-modular-list-excerpt").html(i.excerpt).addTo(
          s,
        ),
        e.element;
    },
    ...
    d = class {
      ...
      waitForIntersection() {
        ...
        new IntersectionObserver((s, n) => {
          this.result === null && s?.[0]?.isIntersecting &&
      ...
      async load() {
        ...
        let e = this.resultFn(this.result), t = g(e);
    ...
    register(e) {
      e.on("search", (t, s) => {
        ...
        e.on("results", (t) => {
          ...
          this.containerEl.innerText = `${s} result${
            s === 1 ? "" : "s"
          } for ${this.term}`;
        }),
        e.on("loading", () => {
  ...
  var h = class {
    ...
    initContainer(e) {
      ...
      let s = `pagefind_modular_filter_pills_${this.filter}`,
        n = new r("div").class("pagefind-modular-filter-pills-wrapper").attrs({
          role: "group",
          "aria-labelledby": s,
        });
      this.alwaysShow || n.attrs({ "data-pfmod-hidden": !0 }),
        new r("div").id(s).class("pagefind-modular-filter-pills-label").attrs({
          "data-pfmod-sr-hidden": !0,
        }).text(`Filter results by ${this.filter}`).addTo(n),
        this.pillContainer = new r("div").class("pagefind-modular-filter-pills")
          .addTo(n),
    ...
    renderNew() {
      this.available.forEach(([e, t]) => {
        new r("button").class("pagefind-modular-filter-pill").html(
          this.pillInner(e, t),
  ...
  var P = async (i = 50) => await new Promise((e) => setTimeout(e, i)), u;
  try {
  ...
  var p = class {
    constructor(e = {}) {
      this.__pagefind__ = null,
        ...
        this.__searchID__ = 0,
        this.__hooks__ = { search: [], filters: [], loading: [], results: [] },
        this.components = [],
        this.searchTerm = "",
        this.searchFilters = {},
        this.searchResult = {},
    ...
    async __search__(e, t) {
      ...
      n && this.__searchID__ === s &&
        (n.filters && Object.keys(n.filters)?.length &&
